...
export default class RoomInfo extends Component {
	...
	onAddRoom = room => {
		...
		if (numberOfRooms.length < roomType.availableRooms) {
			...
			this.props.setStateValue({ totalCharge });
	...
	removeRoom = index => {
		...
		let { checkInDate, checkOutDate } = this.props.values;
		let numberOfDays = checkOutDate.diff(checkInDate, "days");
		let totalCharge = newSelectedRoom.reduce((output, room, i) => {
			console.log(room);
			let roomRate = numberOfDays * room.rate.price;
			return output + roomRate;
		}, 0);
			...
			this.setState({
				snackBarMessage: (
					<span>
						...
						<strong style={{ color: "#f50057" }}>same</strong> or <strong style={{ color: "#f50057" }}>before</strong> the checkin
						date!
					</span>
				),
				snackBar: true
					...
					<Grid container spacing={1}>
						...
						<Grid item xs={12} sm={7}>
							...
							>
								...
								>
									Available Room(s): {selectedType.availableRooms} {this.state.selectedType.name}
								</Typography>
