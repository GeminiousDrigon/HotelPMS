		...
		return (
			<Dialog open={open} maxWidth="md" fullWidth aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
				...
				<DialogContent>
					...
					<input onChange={this.handleFileChange} type="file" style={{ display: "none" }} ref={el => (this.fileInput = el)} accept="image/x-png,image/jpeg" multiple="multiple" />
					<div style={{ display: "flex", flexDirection: "row", marginTop: 30, flexWrap: "wrap" }}>
						{this.state.files.map((el, i) => {
							return <Image el={el} bookingId={this.props.bookingId} uploading={this.state.uploading} index={i} updateStatus={this.updateStatus} />;
						})}
					</div>
				</DialogContent>
				<DialogActions>
...
class Image extends Component {
	...
	submitFile = async () => {
		try {
			...
			await POST(`/api/booking/${this.props.bookingId}/billing/${this.props.billingId}/file`, form, {
				headers: {
			...
			// console.log(file);
			// await axios.post(`/api/bookingtype/${this.props.bookingId}/file`, file, { headers: { "X-Requested-With": "XMLHttpRequest" } });
		} catch (err) {
